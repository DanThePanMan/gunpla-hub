...
const PostCard = ({ post }) => {
  ...
  useEffect(() => {
    if (user && post.likedBy) {
      const hasLiked = post.likedBy.some(
        (like) => like.userId === user.id || like.likedUser?.userId === user.id,
      );
      setLiked(hasLiked);
    }
  }, [user, post.likedBy]);
  ...
  // Carousel navigation
  const hasPictures = Array.isArray(post.pictures) && post.pictures.length > 0;
      ...
      >
        ...
        {hasPictures && (
          ...
          >
            <img
              ...
              onLoad={
                carouselIdx === 0
                  ? (e) => setFirstImgHeight(e.target.offsetHeight)
                  : undefined
              }
            />
                  ...
                  {post.pictures.map((_, idx) => (
                    <span
                      ...
                      className={`inline-block w-2 h-2 rounded-full ${
                        carouselIdx === idx ? "bg-white" : "bg-slate-500"
                      }`}
                    />
                  ))}
                </div>
        ...
        <div className="flex items-center mb-3">
          ...
          >
            ...
            <div className="ml-3">
              <h3 className="text-slate-200 font-semibold hover:text-primary transition-colors">
          ...
          <div className="bg-slate-700 rounded p-3 mb-3 border border-slate-600">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="text-slate-200 ml-2">{post.build.grade}</span>
              </div>
            ...
            {post.build.customizations && (
              <div className="mt-2">
                <span className="text-slate-400 text-sm">Customizations:</span>
